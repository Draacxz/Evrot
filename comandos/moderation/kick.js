...
module.exports = (client) => {
	...
	// Apenas tudo estética.
		...
		const sucessReason = new MessageEmbed()
			.setColor('#30FF00')
			.setDescription(`<:check:829822821337268255> O usuário informado acaba de ser expulsado pelo motivo: ***${reason}***.`);
		
		// Verificando se o comando for igual à String dita abaixo.
		if(command === 'kick' || command === 'expulsar' || command === 'kickar') {

			// Código utilizado para obter o usuário que foi marcado.
			const target = message.mentions.members.first();

			// Verificando se o bot tem as permissões necessárias
			if(!message.guild.me.hasPermission('KICK_MEMBERS')) return message.channel.send(`${message.author}`, guiaBot);
